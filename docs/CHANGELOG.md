...
### Added
- **Transparent background mode — `Ctrl+Alt+T` (also Settings → Background).** A new toggle that hides *every* background layer at once: the blurred album-art backdrop, the album-art color tint, the background-color paint, AND the window's acrylic/mica frost (it drops the Windows DWM backdrop to None). The result is just the lyrics, album art, and media info floating directly over the desktop with nothing behind them. This restores the fully-see-through look. Press `Ctrl+Alt+T` to flip it on/off on the fly; the choice persists across restarts.
- **Hide the media info column — `Ctrl+Alt+H` (also Settings → Extras).** Toggles the right-hand block (track title · progress bar · source badge) so you can run a clean lyrics-only overlay. When the column is hidden, the faint Hum logo watermark slides to the right (into the space the column vacated) with a smooth 280ms animation, and slides back when the column returns. Persists across restarts. (`Ctrl+Alt+M` was the original pick but it's commonly already taken by other software, so it's `H` for "Hide".) The existing `Ctrl+Alt+B` still independently toggles just the blurred album-art layer.

### Fixed
- **Long lyric lines no longer get cut off or overlap the media column.** A long current line used to spill out of the lyrics area and paint over the right-hand media info, becoming unreadable, or hard-truncate with an ellipsis. Now each line is constrained to its own column, and the active line **shrinks its font just enough to fit on one line** (full text, no truncation) instead of cutting off — measured against the real rendered width so it accounts for the font and letter-spacing. The album art and window height stay stable (still one line) so nothing jitters. The media info column was also slimmed (max width 38→20 characters) so lyrics get the lion's share of the width.
- **Resizing the overlay now scales as one piece.** Dragging a top or bottom edge used to change the height independently and then snap back, causing a jarring visual glitch. Now a vertical drag produces the exact same proportional zoom as dragging a side edge: the whole overlay (text + window) grows or shrinks together. Width still drives the scale; a vertical drag is translated into the equivalent width change so the behavior is identical from any edge.
- **Lyrics + album art now resolve for songs with decorative emoji or reversed "Song - Artist" titles.** Two failure modes that made even hugely popular songs show "no lyrics": (1) a trailing decorative emoji on the video title (e.g. `Stay 🎵`, `♪`, `►`) rode through the cleaner into the search query and broke the match — titles now strip leading/trailing emoji, music notes, arrows, and similar symbols (interior punctuation like `P!nk` is preserved); (2) some lyric channels (e.g. "Pillow") title videos *song-first* — `Hanging By A Moment - Lifehouse (Lyrics)` is song-then-artist, the reverse of the usual artist-then-song — so the lookup searched the wrong half and missed. The lyric search now tries **both** halves of an `X - Y` title (the duration check prevents false matches on normal titles), and the album-art lookup gained the same reversed-order variant. Known limitation: the media info column may still show the title/artist swapped for these song-first channels (there's no reliable signal for *display* order), but lyrics and cover art now resolve.

### Changed
- **Overlay window title is now "Hum Lyrics Overlay" (was "Hum").** No visible change (the overlay is borderless and hidden from the taskbar) — this is window metadata. Game anti-cheats (EAC, BattlEye) enumerate window titles for server-side review, so an unambiguous name plus the `hum.exe` process makes Hum clearly identifiable as a lyrics tool rather than an anonymous always-on-top window. Hum injects no code into games, hooks no rendering, and reads no game memory, so it's in the same safe category as OBS / Rainmeter; this is name hygiene on top of that.

## [0.13.43] - 2026-06-09

### Fixed
- **YouTube lyrics actually load now (the real fix).** v0.13.42 cleaned up YouTube's artist/song text but lyrics still showed "Track info unavailable from this source — … cannot be decoded by Hum" (the dev console reported the lyrics source as `unsupported-source`). Root cause: Hum has a guard that refuses to look up lyrics when a browser source's now-playing title is a useless placeholder rather than a song — that guard exists for Pandora (whose title is "{station} - Now Playing on Pandora"). It was wrongly catching **all** YouTube tracks too, because the YouTube watcher claims every Chrome tab as "its" source but only produces real data during ad breaks. So a normal YouTube song was flagged "can't decode this" and the lyric lookup was skipped entirely, before it ever tried. YouTube is now exempt from that guard: its title IS the song (just dressed up with "- Lyrics" / "(Official Video)" etc.), so the lyric resolver runs its normal cleanup-and-search on it. Combined with v0.13.42's artist/song parsing, YouTube songs now show the right artist and title, find synced lyrics via LRCLib, and load album art. Pandora and the video services (Netflix/Twitch/etc.) still correctly show "unavailable" when their bridge has no real song to report. Technical: new `WebPlayerProbe::smtc_unreliable_without_bridge()` (default `true`, overridden to `false` in `YouTubeProbe`) + `web_bridge::any_unreliable_probe_detects`; the lyrics resolver's `unreliable` short-circuit now consults that instead of `any_probe_detects`.
- **YouTube album covers now load, including for background tabs and "- Topic" auto channels.** YouTube songs showed no album art in many cases — the cover area stayed empty. Hum was skipping its own album-art lookup for any browser source (deferring to the YouTube watcher), but that watcher only fetches art when the YouTube tab is the *foreground* tab of its window. So watching the overlay while the song played in a background tab — or playing an auto-generated "Artist - Topic" track — left the cover unfetched by anyone. Hum now fetches the cover itself for these cases, cleaning the metadata first (stripping "- Topic"/VEVO from the artist and "(Lyrics)"/"(Official Video)" from the title, and splitting "Artist - Song" video titles) so iTunes/Deezer return the real cover; it falls back to the video's own thumbnail on a miss. Foreground YouTube still uses the watcher's cover as before. Technical: `smtc::spawn_art_fetch` now defers to the bridge only for unreliable sources (Pandora) or foreground YouTube (`youtube_window_shows_track`), and runs its iTunes query through `clean_artist`/`clean_title`; the cached payload stays keyed to the raw title the overlay matches against.
- **YouTube lyrics appear faster.** On a new YouTube track the lyric lookup used to grind for many seconds before showing anything, because Hum first searched with the raw channel name as the artist (a guaranteed miss that burned the full LRCLib + SimpMusic + NetEase fallback chain) and only then, ~2s later, retried with the real artist once the watcher had parsed it. Hum now briefly waits (up to 2.5s) for the watcher's cleaned "Artist - Song" before the first lookup, so it hits LRCLib's exact-match endpoint on the first try. The overlay shows a "fetching" state during the short wait. Non-YouTube sources and background-tab YouTube are unaffected (they resolve immediately). Technical: a wait-for-bridge gate in the lyrics worker, gated on `youtube_window_shows_track` and time-bounded so it never hangs.
