...
- **A failed primary lyric source is reported to the user as an authoritative "no lyrics found".** In `lyrics.rs::resolve_lyrics`, when LRCLib errors and NetEase cleanly misses, `any_clean_notfound` wins and the overlay shows the not-found state. The user cannot tell "this song has no lyrics anywhere" from "the lyrics service was unreachable", which is why the SNI block above read as a title-matching bug for weeks. An errored primary should not let a peer's miss present as authoritative. Owned by the next slice.
