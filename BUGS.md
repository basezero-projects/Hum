...
- **Auto-update banner (`UpdateBanner`) only renders in the 3-line layout.** `src/Overlay.tsx` mounts `<UpdateBanner .../>` only inside the three-line branch's `outerStack` (~line 991); the `single_line` and `full_page` branches never render it, so a user on those layouts gets no in-overlay "update available / installing" prompt (the tray "Check for updates" still works). Same class as the v0.13.41 NudgeBanner fix, but UpdateBanner is a flow element (not absolutely positioned), so it needs a deliberate placement in each layout rather than a drop-in. Noticed during the 2026-06-03 bug sweep; left out of that batch to avoid layout scope-creep.
