...
- **LRCLib HTTPS requests can fail locally with a corrupt TLS content-type error.** During v0.13.64 native release QA, both LRCLib lookup and search reported `received corrupt message of type InvalidContentType` for multiple tracks while other app networking continued. Confirm whether this is local proxy or certificate interception before changing the lyric client, then add a transport regression if it reproduces on a clean network.
