...
- (was) **The update banner cannot be dismissed.** The dot and label sit on an always-on-top window over whatever the user is watching, and the only way to clear an "available" state was to install. Fixed with a hover close control on the states that persist ("available" and errors), keyed per notice so dismissing one version does not silence the next. The Ghost-mode click-through hole was reading a different visibility rule and would have kept swallowing clicks over a dismissed banner, so both now share `bannerIsVisible`.
- (was) **Installing an update could cut off a song with no warning.** `install()` exits the process immediately, so the click that triggered it ended playback mid-line. Now a click during playback enters a `confirming` state that names the consequence and needs a second click, reverting after ten seconds so it cannot decay into a one-click restart. Note for anyone revisiting this: a true "install later" is not available, because `tauri-plugin-updater` calls `std::process::exit(0)` inside `install()` on Windows (`updater.rs:865`).
