...
- **The hidden Settings webview can load before Rust manages settings state.** Native v0.13.65 QA captured `state not managed` in the predeclared Settings window during startup. When that race occurs, the first later opening shows the recoverable Settings error page instead of retrying automatically. Delay the initial request until setup completes or reuse the production retry helper in HUM-10F.
